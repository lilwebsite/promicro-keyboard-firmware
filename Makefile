...
DEFINES=-DF_CPU=$(FREQ) -DBAUD=$(BAUD) -DKEY_ROLLOVER=$(KEY_ROLLOVER) -DENDPOINT_SIZE=$(ENDPOINT_SIZE) -DPACKET_SIZE=$(PACKET_SIZE) -DREPORT_COUNT=$(REPORT_COUNT) -DDEMUX_OUT=$(DEMUX_OUT) -DDEMUX_IN=$(DEMUX_IN) -DCOLUMNS=$(COLUMNS) -DROWS=$(ROWS) -DKEYS=$(KEYS) -D$(DRIVER)
